...
class NQOpenDataset(BaseDataset):
    """Natural Questions - Open dataset implementation focused on data processing."""
    ...
    def load_dataset(self, path: str, shard_index: int = 0, num_shards: int = 1) -> List[Dict[str, Any]]:
        """Load NQ Open dataset from JSONL file and return data for specific shard."""
        try:
            all_data = []
            ...
            print(f"NQ Open dataset loaded with {len(all_data)} entries")
            ...
            raise RuntimeError(f"Failed to load NQ Open dataset from {path}: {e}")
    ...
    def get_dataset_size(self, path: str) -> int:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to get NQ Open dataset size from {path}: {e}")
    
    def apply_template(self, entry_data: str, template_provider) -> str:
        """Apply template to Natural Questions data."""
        # For Natural Questions, we just pass the question as is
...
# Register the dataset with multiple names for compatibility
DatasetRegistry.register("nq_open", NQOpenDataset)
DatasetRegistry.register("natural_questions", NQOpenDataset)
